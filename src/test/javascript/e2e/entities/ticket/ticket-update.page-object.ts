...
export default class TicketUpdatePage {
  pageTitle: ElementFinder = element(by.id('myApp.ticket.home.createOrEditLabel'));
  ...
  async enterData() {
    ...
    await waitUntilDisplayed(this.saveButton);
    await this.setDueDateInput('01-01-2001');
    await waitUntilDisplayed(this.saveButton);
    const selectedDone = await this.getDoneInput().isSelected();
    if (selectedDone) {
      await this.getDoneInput().click();
    } else {
      await this.getDoneInput().click();
    }
